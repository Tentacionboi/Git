...
Status: In progress. Market baseline exists, and a deterministic Elo rating engine now produces historical rating tables plus first-pass and calibrated 1X2 probabilities. World Cup-only Elo evaluation, draw calibration, model-vs-market comparison code, and odds timing validation exist; verified real historical World Cup odds are still pending.

Deliverables:

- Market baseline.
- Elo model. Rating engine, first-pass 1X2 probability split, World Cup evaluation, and draw calibration done.
- Model-vs-market comparison. Code and no-leakage timing validation done for supplied odds snapshots; real historical World Cup odds pending.
- Poisson goals model.
